...
def filter_by_length(tensor_list, min_len_list=None, max_len_list=None):
    ...
    if min_len_list:
        if len(min_len_list) != len(tensor_list):
            raise ValueError(
                "Min length list need to match size of tensor_list.")
    else:
    ...
    if max_len_list:
        if len(max_len_list) != len(tensor_list):
            raise ValueError(
                "Max length list need to match size of tensor_list.")
    else:
    ...
    for min_len, max_len, tensor in zip(min_len_list, max_len_list, tensor_list):
        ...
        if any([min_len, max_len]):
            tensor_len = tf.reduce_sum(
                tf.cast(tf.greater(tensor, 0), tf.int32))
            if min_len:
