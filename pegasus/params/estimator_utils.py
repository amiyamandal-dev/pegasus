...
def _tile_scalar_to_batch_size(tensor_dict, batch_size):
    ...
    scalar_keys = [k for k, v in tensor_dict.items() if len(
        v.shape) == 0]  # pylint: disable=g-explicit-length-test
    tiled_dict = {}
...
def add_scalars_to_summary(summary_dir, scalar_tensors_dict):
    ...
    def host_call_fn(**kwargs):
        writer = contrib_summary.create_file_writer(
            summary_dir, max_queue=1000)
